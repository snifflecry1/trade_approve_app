...
class TestTradeService:
    ...
    def test_submit_trade_for_approval_valid(self, setup_service_with_trade):
        ...
        result = setup_service.submit_trade_for_approval(
            trade_id=trade_id, user_id=123, note="Submitting for approval"
        )
    ...
    def test_submit_trade_for_approval_invalid_id(self, setup_service):
        ...
        result = setup_service.submit_trade_for_approval(
            trade_id=999, user_id=123, note="Submitting for approval"
        )
    ...
    def test_approve_trade_valid(self, setup_service_with_trade):
        ...
        setup_service.submit_trade_for_approval(
            trade_id=trade_id, user_id=123, note="Submitting for approval"
        )
        result = setup_service.approve_trade(
            trade_id=trade_id, user_id=456, note="Approving trade"
        )
        assert result is True
    ...
    def test_approve_trade_incorrect_user_id(self, setup_service_with_trade):
        ...
        result = setup_service.approve_trade(
            trade_id=trade_id, user_id=123, note="Approving trade"
        )
        assert result is False
    ...
    def test_cancel_trade_valid(self, setup_service_with_trade):
        ...
        result = setup_service.cancel_trade(
            trade_id=trade_id, user_id=789, note="Cancelling trade"
        )
        assert result is True
        log_entry = setup_service.action_log.action_log[trade_id][1]
        assert log_entry.trade_id == trade_id
    ...
    def test_cancel_trade_invalid_id(self, setup_service):
        ...
        result = setup_service.cancel_trade(
            trade_id=999, user_id=789, note="Cancelling trade"
        )
        assert result is False
    ...
    def test_cancel_trade_already_cancelled(self, setup_service_with_trade):
        ...
        setup_service.cancel_trade(
            trade_id=trade_id, user_id=789, note="Cancelling trade"
        )
        result = setup_service.cancel_trade(
            trade_id=trade_id, user_id=789, note="Cancelling trade again"
        )
        assert result is False
    ...
    def test_update_trade_valid(self, setup_service_with_trade):
        ...
        updates = {"notion_amount": 20000.0}
        result = setup_service.update_trade(
            trade_id=trade_id, user_id=456, note="Updating trade", **updates
        )
        assert result is True
    ...
    def test_update_trade_invalid_updater(self, setup_service_with_trade):
        ...
        updates = {"notion_amount": 20000.0}
        result = setup_service.update_trade(
            trade_id=trade_id, user_id=123, note="Updating trade", **updates
        )
        assert result is False
    ...
    def test_send_trade_to_counterparty_valid(self, setup_service_with_trade):
        ...
        setup_service.approve_trade(
            trade_id=trade_id, user_id=456, note="Approving trade"
        )
        result = setup_service.sent_trade_to_counterparty(
            trade_id=trade_id, user_id=456, note="Sending to counterpary"
        )
        assert result is True
        ...
        assert (
            setup_service.trade_history.store[trade_id][State.EXECUTED].strike
            is not None
        )
    ...
    def test_send_trade_to_counterparty_invalid_user(self, setup_service_with_trade):
        ...
        setup_service_with_trade.submit_trade_for_approval(
            trade_id=trade_id, user_id=123, note="Submitting for approval"
        )
        setup_service_with_trade.approve_trade(
            trade_id=trade_id, user_id=456, note="Approving trade"
        )
        result = setup_service_with_trade.sent_trade_to_counterparty(
            trade_id=trade_id, user_id=123, note="Sending to counterpary"
        )
        assert result is False
    ...
    def test_book_trade_valid(self, setup_service_with_trade):
        ...
        setup_service.sent_trade_to_counterparty(
            trade_id=trade_id, user_id=456, note="Sending to counterpary", strike=1.25
        )
        log_entry = setup_service.action_log.get_latest_log(trade_id)
        assert log_entry.to_state == State.SENT_COUNTERPARTY
        result = setup_service.book_trade(
            trade_id=trade_id, user_id=123, note="Booking trade"
        )
        assert result is True
        assert State.EXECUTED in setup_service.trade_history.store[trade_id]
        log_entry = setup_service.action_log.get_latest_log(trade_id)
    ...
    def test_view_trade_version(self, setup_service_with_trade):
        ...
        trade_version = setup_service.view_trade_details(
            trade_id=trade_id, state=State.PENDING_APPROVE
        )
    ...
    def test_view_trade_state_diff(self, setup_service_with_trade):
        ...
        diffs = setup_service.compare_trade_versions(
            trade_id=trade_id, state1=State.DRAFT, state2=State.EXECUTED
        )
