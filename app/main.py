...
def cmd_send_to_counterparty(args):
    ...
    if success:
        ...
        if latest_log:
            ...
            print(f"✓ Trade {args.trade_id} sent to counterparty and executed at strike {trade.strike}")
        else:
...
def cmd_view_trade_details(args):
    ...
    try:
        ...
        print(f"✗ Invalid state: {args.state}. Valid states: {', '.join([s.name for s in State])}")
        sys.exit(1)
...
def cmd_compare_trade_versions(args):
    ...
    try:
        ...
        print(f"✗ Invalid state: {e}. Valid states: {', '.join([s.name for s in State])}")
        sys.exit(1)
    
    diffs = service.compare_trade_versions(trade_id=args.trade_id, state1=state1, state2=state2)
    if diffs:
        ...
        print(f"No differences found between states {state1.name} and {state2.name} for trade {args.trade_id}.")
...
def main():
    ...
    draft_parser.add_argument('--direction', required=True, choices=['BUY', 'SELL'], help='Trade direction')
    draft_parser.add_argument('--style', required=True, help='Instrument style (e.g., FORWARD, OPTION)')
    draft_parser.add_argument('--notion-curr', required=True, help='Notional currency (e.g., USD, EUR)')
    draft_parser.add_argument('--notion-amount', type=float, required=True, help='Notional amount')
    draft_parser.add_argument('--underlying', nargs=2, required=True, help='Two underlying currencies')
    draft_parser.add_argument('--t-date', required=True, help='Trade date (YYYY-MM-DD)')
    draft_parser.add_argument('--v-date', required=True, help='Valuation date (YYYY-MM-DD)')
    draft_parser.add_argument('--d-date', required=True, help='Delivery date (YYYY-MM-DD)')
    ...
    submit_parser.add_argument('--user-id', type=int, required=True, help='User ID submitting')
    ...
    approve_parser.add_argument('--user-id', type=int, required=True, help='User ID approving')
    ...
    cancel_parser.add_argument('--user-id', type=int, required=True, help='User ID cancelling')
    ...
    update_parser.add_argument('--user-id', type=int, required=True, help='User ID updating')
    update_parser.add_argument('--note', help='Optional note')
    update_parser.add_argument('--notion-amount', type=float, help='New notional amount')
    ...
    send_parser = subparsers.add_parser('send-to-counterparty', 
    ...
    send_parser.add_argument('--user-id', type=int, required=True, help='User ID sending')
    send_parser.add_argument('--note', help='Optional note')
    send_parser.add_argument('--strike', type=float, help='Execution strike price (optional, will simulate if not provided)')
    ...
    book_parser.add_argument('--user-id', type=int, required=True, help='User ID booking')
    book_parser.add_argument('--note', help='Optional note')
    book_parser.set_defaults(func=cmd_book)
    
    # view-trades command
    view_parser = subparsers.add_parser('view-trades', 
        help='View all trades and their states',
        usage='view-trades')
    view_parser.set_defaults(func=cmd_view_trades)
    
    # view-action-log command
    log_parser = subparsers.add_parser('view-action-log', 
        help='View action log for a specific trade',
        usage='view-action-log 1')
    log_parser.add_argument('trade_id', type=int, help='Trade ID to view action log for')
    log_parser.set_defaults(func=cmd_view_action_log)
    
    # view-trade-states command
    states_parser = subparsers.add_parser('view-trade-states', 
    ...
    details_parser = subparsers.add_parser('view-trade-details', 
    ...
    details_parser.add_argument('state', help='State to view (e.g., DRAFT, PENDING_APPROVE, APPROVED, EXECUTED)')
    details_parser.set_defaults(func=cmd_view_trade_details)
    
    # compare-trade-versions command
    compare_parser = subparsers.add_parser('compare-trade-versions', 
