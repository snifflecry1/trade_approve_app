...
class TestActionLog:
    ...
    def test_record_multiple_trades_separate_logs(self, action_log_instance):
        ...
        action_log_instance.record(
            trade_id_1, user_id, Action.SUBMIT, State.DRAFT, "T1 submitted"
        )
        action_log_instance.record(
            trade_id_2, user_id, Action.SUBMIT, State.DRAFT, "T2 submitted"
        )
        action_log_instance.record(
            trade_id_1, user_id, Action.APPROVE, State.PENDING_APPROVE, "Approved"
        )
